...
# intents_file_path = "C:/Users/emale/Documents/uni/MA/bt_crs/conv_rec_sys_food/NLU/samples_intent_classification/intents/v_20221205/"
# intents_file_path = "C:/Users/emale/Documents/uni/MA/bt_crs/conv_rec_sys_food/NLU/samples_intent_classification/intents/v_20221206/"
# intents_file_path = "C:/Users/emale/Documents/uni/MA/conv_rec_sys_food/NLU/samples_intent_classification/intents/v_20230120/"
intents_file_path = "C:/Users/emale/Documents/uni/MA/conv_rec_sys_food/NLU/samples_intent_classification/intents/v_20230207/"

# intents_file_name = "v_20221205_all_intents.csv"

# intents_file_name = "v_20221206_all_intents.csv"
# intents_file_name = "v_20221206_all_intents_lowered.csv"
# intents_file_name = "v_20221206_all_intents_lower_upper_combined.csv"

# intents_file_name = "v_20230120_all_intents_lowered.csv"
# intents_file_name = "v_20230120_all_intents_lower_upper_combined.csv"
# intents_file_name = "v_20230120_all_intents.csv"

intents_file_name = "v_20230207_all_intents.csv"
...
lr = 1e-4
dropout = 0.5
model_name = "german_cased_bert_no_placeholders_" + str(datetime.now().strftime("%Y%m%d_%H%M%S"))
model_file_path = "C:/Users/emale/Documents/uni/MA/bt_crs/conv_rec_sys_food/NLU/saved-models/"

_use_ne_placeholders = False
_save_plot = True
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, df):
        ...
        self.labels = list(df.intent_code)
...
def _read_intent_file(file):
    intent_df = pd.read_csv(file, sep=",", header=0)
    return intent_df
...
def train(model, train_data, val_data, learning_rate, epochs):
    ...
    for epoch_num in range(epochs):
        ...
        for train_input, train_label in tqdm(train_dataloader):
            ...
            # tb_writer.add_scalar("training_loss", batch_loss, global_step=step)
            # tb_writer.add_scalar("training_accuracy", float(acc)/float(train_label.shape[0]), global_step=step)
    ...
    # Help for interpreting plots: https://machinelearningmastery.com/learning-curves-for-diagnosing-machine-learning-model-performance/
    # [08.12.2022]
...
def evaluate(model, test_data):
    ...
    # tb_writer.add_scalar("f1_score", f1_score)
...
def save_model(model):
    ...
    #    pickle.dump(model, f)
    # tb_writer.add_scalar("model_name", file_name)
    # print("model_name: ", file_name)


def main():

    print("Reading intent files...")
    intent_df = _read_intent_file(intents_file_path + intents_file_name)
    intent_df = intent_df[["text", "intent_code"]]
    # pd.set_option('display.max_columns', None)
    # print(intent_df.head())
    # print(intent_df.intent_code.value_counts())
    np.random.seed(814)

    second_split_limit = train_size + (1-train_size)/2

    df_train, df_val, df_test = np.split(intent_df.sample(frac=1, random_state=612),
                                         [int(train_size * len(intent_df)), int(second_split_limit * len(intent_df))])

    print("train_length:", len(df_train))
    print("validate_length:", len(df_val))
    print("test_length: ", len(df_test))

    print("train_counts:", df_train.intent_code.value_counts())
    print("val_counts:", df_val.intent_code.value_counts())
    print("test_counts:", df_test.intent_code.value_counts())
